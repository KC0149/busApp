...
import DispatchContext from "./DispatchContext.tsx"
...
function App() {
  const [server, setServer] = useState<string>("Wait")
  const [vehRef, setVehRef] = useState<string[]>([])
  const [publishLine, setPublishLine] = useState<string[]>([])
  const [axisType, setAxisType] = useState<string[]>([])

  const [isloading, setisLoading] = useState<boolean>(false)

  const appDispatch = useContext(DispatchContext)! //confirm not null
  const appState = useContext(StateContext)!

  interface State {
    vehRefHistory: string[]
    publishlineHistory: string[]
    axisTypeHistory: string[]
  }

  interface Action {
    type: "saveHistory" | "clearHistory"
    value?: string // For "saveHistory" action, value will be a string
  }
  ...
  function ourReducer(draft: State, action: Action) {
    switch (action.type) {
      case "saveHistory":
        let key: keyof State, history: string
  ...
  //to show on map
  async function handleChangeVehref(vehref: string) {
    if (vehRef) {
      setisLoading(true)
  ...
  async function handleChangePubLine(publishLine: string) {
    setisLoading(true)
  ...
  async function handleChangeAxisType(axisType: string) {
    setisLoading(true)
    ...
    <div className="w-full min-h-screen flex flex-col items-center">
      ...
      <div className="w-full  overflow-hidden ">
        ...
        <div className={`map  z-1 transition-all ease-in-out duration-300  ${appState.taskbar ? "w-screen" : "w-full"}`}>
          <Map geojson={appState.toggleGeoJson ? appState.combineGeoJson : appState.geoJson} setisLoading={setisLoading} />
        </div>
        <div
        ...
        >
          <button
            ...
            onClick={() => appDispatch({ type: "taskbar", value: appState.isChecked ? false : true })}
          >
