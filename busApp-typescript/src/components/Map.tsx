...
import { Feature, FeatureProperties, GeojsonType } from "./Geojson"
...
interface FeatureMap {
  [pointIndex: number]: geometry
}

const Map: React.FC<MapProps> = ({ geojson, setisLoading }) => {
  const [center, setCenter] = useState<L.LatLngExpression>([40.689916, -73.930448])
  const key = JSON.stringify(geojson) // need the key to rerender the map
  const mapRef = useRef<L.Map>(null)
  const [allProperties, setAllProperties] = useState<FeatureProperties[]>([])
  const geojsonLayerRef: RefObject<L.GeoJSON<any, any>> = useRef(null)
  ...
  useEffect(() => {
    ...
    const calculateCenter = () => {
      if (geojson && geojson.features && geojson.features.length > 0) {
        ...
        return [40.689916, -73.930448] as L.LatLngExpression // Default center if no GeoJSON data or features
      }
    }
  ...
  useEffect(() => {
    if (!appState.toggleGeoJson && geojson && appState.runBusRouting) {
      ...
      const geometries: FeatureMap[] = []
      geojson.features.forEach((feature: Feature, index: number) => {
        const featureMap: FeatureMap = {}
        if (feature.geometry.type === "LineString") {
          // Extract coordinates from the geometry
          const coordinates: Array<number[] | number> = feature.geometry.coordinates
          ...
          coordinates.forEach((coordinate, pointIndex) => {
            ...
            const [long, lat] = coordinate as [number, number]
      ...
      async function busRouting() {
        try {
                  ...
                  while (response === null || response.data === "wait") {
                    ...
                    if (response.data !== "wait") {
                      if (i === 0 && Object.keys(pointsArray).length === 2) {
                        response.data.features = response.data.features.filter((feature: Feature) => {
                        ...
                        response.data.features = response.data.features.filter((feature: Feature) => feature.geometry.type === "LineString")
                      }
          ...
          combinedFeatures.forEach((feature: Feature) => {
  ...
  interface CustomMarker extends L.Marker {
    _icon: HTMLElement // Define the type of _icon property
  }
  interface CustomLayer extends L.Layer {
    setStyle: (style: object) => HTMLElement
    feature: Feature
  }
  type CustomLayerOrMarker = CustomMarker | CustomLayer
  let prevLayer: CustomLayerOrMarker

  // Define a type guard function to check if the layer is a CustomMarker
  function isMarker(layer: L.Layer): layer is CustomMarker {
    return (layer as CustomMarker)._icon !== undefined
  }
  //set route coloring when clicked
  function colorLayer(layer: CustomLayer) {
    console.log(layer)
    //reset previous selected color
    if (prevLayer) {
      if (isMarker(prevLayer)) {
  ...
  useEffect(() => {
    if (!appState.infoVisible) {
      geojsonLayerRef.current?.eachLayer((layer: L.Layer) => {
        const customLayerOrMarker = layer as CustomLayerOrMarker // Type assertion

        if (isMarker(customLayerOrMarker)) {
          customLayerOrMarker._icon.classList.remove("huechange")
        } else {
          customLayerOrMarker.setStyle({ color: "#3388ff" })
        }
      })
    }
  }, [appState.infoVisible])
  ...
  return (
    <div className="relative" style={{ width: "100%", height: "100%", position: "unset" }}>
      <MapContainer ref={mapRef} center={center} zoom={10} scrollWheelZoom={true}>
        <TileLayer
        ...
        <GeoJSON
          ...
          key={key}
          data={geojson}
          onEachFeature={function (feature, layer: CustomLayer) {
            layer.on("click", e => {
              colorLayer(layer)
            })
          }}
